...
with open(input_file, 'r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        # If we haven't seen this ContractPlanID for this state, add it
        if contract_plan_id not in state_plans[state]:
            # Generate URL by replacing underscore with dash
            # Format: 2026-H5521-296-0 from H5521_296 and segment 0
            ...
            state_plans[state][contract_plan_id] = {
                "State": row['State Territory Name'],
            ...
            if county == "All Counties":
                state_plans[state][contract_plan_id]["all_counties"] = True
...
for state, plans in sorted(state_plans.items()):
    ...
    print(f"{state:30} - {len(plans_list):4} unique ContractPlanIDs -> {safe_state_name}.json")
