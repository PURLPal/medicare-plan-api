#!/usr/bin/env python3
"""
Build New Jersey ZIP files using the EXACT SAME standard format as Arkansas/MA/SC.
Only processes New Jersey ZIPs (07xxx-08xxx) to avoid wasteful regeneration.
"""
import json
...
def load_cms_categories():
    ...
    nj_file = state_data_dir / 'New_Jersey.json'
    if nj_file.exists():
        categories = {}
        ...
        state_categories['New_Jersey'] = categories
    
    return state_categories

def build_new_jersey_zips():
    """Build New Jersey ZIP files in standard format."""
    print("\n" + "="*80)
    print("BUILDING NEW JERSEY IN STANDARD FORMAT")
    print("="*80)
    ...
    # Load New Jersey mock API data
    mock_api_dir = Path('mock_api/NJ')
    with open(mock_api_dir / 'zip_to_plans.json') as f:
        nj_zip_to_plans = json.load(f)
    
    # Load scraped plans
    nj_plans = {}
    scraped_dir = Path('scraped_json_all')
    for plan_file in scraped_dir.glob('New_Jersey-*.json'):
        with open(plan_file) as f:
            plan_data = json.load(f)
            plan_id = plan_data.get('plan_id')
            if plan_id:
                nj_plans[plan_id] = plan_data
    
    print(f"Loaded {len(nj_plans)} New Jersey plans")
    
    # Load CMS categories
    state_categories = load_cms_categories()
    nj_categories = state_categories.get('New_Jersey', {})
    print(f"Loaded {len(nj_categories)} category mappings")
    
    timestamp = datetime.utcnow().isoformat() + 'Z'
    
    # Filter to only New Jersey ZIPs (07xxx-08xxx)
    nj_zips = [z for z in unified_zip.keys() if z.startswith('07') or z.startswith('08')]
    print(f"\nFound {len(nj_zips)} New Jersey ZIPs to generate")
    
    zip_dir = OUTPUT_DIR / 'zip'
    zip_dir.mkdir(parents=True, exist_ok=True)
    
    count = 0
    for zip_code in sorted(nj_zips):
        ...
        for county in zip_info.get('counties', []):
            county_entry = {
                ...
                'fips': county['fips'],
                'state': county['state'],
                'plans_available': county['state'] == 'New Jersey'
            }
            counties_data.append(county_entry)
        
        # Get New Jersey plans for this ZIP
        plan_ids = nj_zip_to_plans.get(zip_code, [])
        
        plans_list = []
        for plan_id in plan_ids:
            if plan_id not in nj_plans:
                continue
            ...
            category = nj_categories.get(plan_id, 'MAPD')
        ...
        count += 1
        if count % 100 == 0:
            print(f"  Generated {count}/{len(nj_zips)} ZIPs...")
    
    print(f"\n{'='*80}")
    print("COMPLETE!")
    print(f"{'='*80}")
    print(f"✅ Generated {count} New Jersey ZIPs in STANDARD format")
