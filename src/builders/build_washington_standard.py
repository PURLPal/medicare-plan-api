#!/usr/bin/env python3
"""
Build Washington ZIP files using the standard format.
Only processes Washington ZIPs (98xxx-99xxx) to avoid wasteful regeneration.
"""
import json
...
def load_cms_categories():
    ...
    wa_file = state_data_dir / 'Washington.json'
    if wa_file.exists():
        categories = {}
        ...
        state_categories['Washington'] = categories
    
    return state_categories

def build_washington_zips():
    """Build Washington ZIP files in standard format."""
    print("\n" + "="*80)
    print("BUILDING WASHINGTON IN STANDARD FORMAT")
    print("="*80)
    ...
    # Load Washington mock API data
    mock_api_dir = Path('mock_api/WA')
    with open(mock_api_dir / 'zip_to_plans.json') as f:
        wa_zip_to_plans = json.load(f)
    
    # Load scraped plans
    wa_plans = {}
    scraped_dir = Path('scraped_json_all')
    for plan_file in scraped_dir.glob('Washington-*.json'):
        with open(plan_file) as f:
            plan_data = json.load(f)
            plan_id = plan_data.get('plan_id')
            if plan_id:
                wa_plans[plan_id] = plan_data
    
    print(f"Loaded {len(wa_plans)} Washington plans")
    
    # Load CMS categories
    state_categories = load_cms_categories()
    wa_categories = state_categories.get('Washington', {})
    print(f"Loaded {len(wa_categories)} category mappings")
    
    timestamp = datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')
    
    # Filter to only Washington ZIPs (98xxx-99xxx)
    wa_zips = [z for z in unified_zip.keys() if z.startswith('98') or z.startswith('99')]
    print(f"\nFound {len(wa_zips)} Washington ZIPs to generate")
    
    zip_dir = OUTPUT_DIR / 'zip'
    zip_dir.mkdir(parents=True, exist_ok=True)
    
    count = 0
    for zip_code in sorted(wa_zips):
        ...
        for county in zip_info.get('counties', []):
            county_entry = {
                ...
                'fips': county['fips'],
                'state': county['state'],
                'plans_available': county['state'] == 'Washington'
            }
            counties_data.append(county_entry)
        
        # Get Washington plans for this ZIP
        plan_ids = wa_zip_to_plans.get(zip_code, [])
        
        plans_list = []
        for plan_id in plan_ids:
            if plan_id not in wa_plans:
                continue
            ...
            category = wa_categories.get(plan_id, 'MAPD')
        ...
        count += 1
        if count % 100 == 0:
            print(f"  Generated {count}/{len(wa_zips)} ZIPs...")
    
    print(f"\n{'='*80}")
    print("COMPLETE!")
    print(f"{'='*80}")
    print(f"✅ Generated {count} Washington ZIPs in STANDARD format")
