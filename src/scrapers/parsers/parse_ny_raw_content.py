#!/usr/bin/env python3
"""
Parse New York raw HTML content into structured JSON.
Based on proven Washington/New Jersey parser.
"""
import json
import re
from pathlib import Path

RAW_DIR = Path('raw_ny_plans')
...
def extract_plan_data(raw_content):
    ...
    org_match = re.search(r'Aetna Medicare|Humana|UnitedHealthcare|Wellcare|Blue Cross Blue Shield|BCBS|Devoted Health|Cigna|CareSource|Health Net|Kaiser Permanente|Clover|Emblem|Oxford|Highmark|EmblemHealth|Healthfirst|VNS Health|MetroPlus|Fidelis', raw_content, re.IGNORECASE)
    if org_match:
...
def parse_plan_file(plan_id):
    ...
    json_file = SCRAPED_DIR / f'New_York-{plan_id}.json'
