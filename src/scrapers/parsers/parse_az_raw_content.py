#!/usr/bin/env python3
"""
Parse raw HTML from Arizona plans to extract structured data.
...
def main():
    print("="*80)
    print("PARSING ARIZONA RAW HTML FILES")
    print("="*80)
    ...
    all_az_files = list(JSON_DIR.glob('Arizona-*.json'))
    with_data = 0
    
    for f in all_az_files:
        with open(f) as fp:
            data = json.load(fp)
        if len(data.get('premiums', {})) > 0 or len(data.get('benefits', {})) > 0:
            with_data += 1
    
    print(f"Total AZ plan files: {len(all_az_files)}")
    print(f"With complete data: {with_data}")
    if len(all_az_files) > 0:
        print(f"Quality: {(with_data/len(all_az_files)*100):.1f}%")
    print(f"{'='*80}\n")
