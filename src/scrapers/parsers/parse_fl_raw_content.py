#!/usr/bin/env python3
"""
Parse raw HTML from Florida plans to extract structured data.
...
def main():
    print("="*80)
    print("PARSING FLORIDA RAW HTML FILES")
    print("="*80)
    ...
    all_fl_files = list(JSON_DIR.glob('Florida-*.json'))
    with_data = 0
    
    for f in all_fl_files:
        with open(f) as fp:
            data = json.load(fp)
        if len(data.get('premiums', {})) > 0 or len(data.get('benefits', {})) > 0:
            with_data += 1
    
    print(f"Total FL plan files: {len(all_fl_files)}")
    print(f"With complete data: {with_data}")
    if len(all_fl_files) > 0:
        print(f"Quality: {(with_data/len(all_fl_files)*100):.1f}%")
    print(f"{'='*80}\n")
